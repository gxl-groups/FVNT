...
import os
from mine.utils import generate_label
def tensor_for_board(img_tensor):
    ...
    if img_tensor.size(1) == 1:
        ...
        tensor = generate_label(img_tensor,img_tensor.size(2),img_tensor.size(3))
    else:
        tensor = img_tensor
        if torch.max(tensor<=1.0):
            tensor = (tensor.clone() + 1) * 0.5
...
def tensor_list_for_board(img_tensors_list):
    ...
    canvas = torch.FloatTensor(batch_size, channel, canvas_h, canvas_w).fill_(0)
...
def save_images_NoNorm(img_tensors, img_names, save_dir):
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    for img_tensor, img_name in zip(img_tensors, img_names):
