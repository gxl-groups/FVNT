...
class DataSet(data.Dataset):
    def __init__(self,opt):
        ...
        self.parsing_path = opt.genetate_parsing
    ...
    def __getitem__(self, item):
        ...
        # source = target.replace('0.jpg','1.jpg')
        ...
        source_name = source.replace('/', '_').replace('.jpg', '')
        target_name = target.replace('/', '_').replace('.jpg', '.png')
        file_name = source_name + target_name
        for i in range(20):
        ...
        target_parsing = Image.open(os.path.join(self.data_root,self.mode,self.parsing_path, file_name))
        target_parsing = np.array(target_parsing).astype(np.long)
        target_im_parsing = torch.from_numpy(target_parsing)
        for i in range(20):
        ...
        target_body_im_hole = target_body_im + target_shoes_im
